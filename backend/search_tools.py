...
class CourseOutlineTool(Tool):
    ...
    def __init__(self, vector_store: VectorStore):
        self.store = vector_store
        self.last_sources = []  # Track sources from last lookup, for UI consistency

    def get_tool_definition(self) -> Dict[str, Any]:
        """Return Anthropic tool definition for this tool"""
        return {
            "name": "get_course_outline",
            "description": "Get the outline of a course: its title, course link, and complete list of lessons (lesson number and title for each). Use for questions about course structure, syllabus, or 'what lessons are in X'.",
            "input_schema": {
                "type": "object",
                "properties": {
                    "course_name": {
    ...
    def execute(self, course_name: str) -> str:
        ...
        Args:
            course_name: Course title to resolve and look up
    ...
    def _format_outline(self, outline: Dict[str, Any]) -> str:
        ...
        lines.append(f"Course Link: {course_link if course_link else 'N/A'}")
        ...
        # Track source for the UI, consistent with CourseSearchTool

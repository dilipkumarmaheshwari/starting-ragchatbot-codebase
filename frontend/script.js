...
function addMessage(content, type, sources = null, isWelcome = false) {
    ...
    if (sources && sources.length > 0) {
        const sourceHtml = sources.map(s => {
            ...
            return s.link
                ? `<a href="${escapeHtml(s.link)}" target="_blank" rel="noopener noreferrer" class="source-badge source-badge-link">${label}</a>`
...
async function startNewChat() {
    if (currentSessionId) {
        try {
            await fetch(`${API_URL}/session/${currentSessionId}/end`, {
                method: 'POST'
            });
        } catch (error) {
